...
def _inject_dropout_to_linear(model, p=MC_DROPOUT_P):
    ...
    ✅ AnomalyDAE 처럼 Dropout 레이어가 아예 없는 모델에
       nn.Linear 의 forward 를 패치하여 functional.dropout 동적 삽입
...
def evaluate_model(
    ...
    checkpoint_file = (
        f"{checkpoint_dir}/{chain}_{model_name}_deepwalk_mc_checkpoint.json"
    )
    ...
    for param in param_list:
        ...
        try:
            ...
            print(
                f"{time_str} --> [Grid] DeepWalk MC {model_name} {param} "
            ...
            print(f"  [ERROR] DeepWalk MC {model_name} | {param} → {e}. Skipping.")
            continue
    ...
    print(f"\n{'─'*60}")
    print(f"[Phase 2] Final Evaluation – DeepWalk MC {model_name}")
    ...
    try:
        ...
        time_str = datetime.now().strftime("%Y:%m:%d_%H:%M:%S")
        print(
            f"{time_str} --> [Final] DeepWalk MC {model_name} "
        ...
        print(f"  [ERROR] Final eval failed for DeepWalk MC {model_name}: {e}")
...
def main():
    ...
    if not results_df.empty:
        results_df.to_csv(
            f'{RESULT_PATH}/mc-main-deepwalk-results_{chain}_raw.csv',
            index=False
        )
    if not final_results_df.empty:
        final_results_df.to_csv(
            f'{RESULT_PATH}/mc-main-deepwalk-results_{chain}_final.csv',
            index=False
        )

    print(f"\n✅ DeepWalk MC 최종 평가 결과가 {RESULT_PATH} 에 저장되었습니다.")
